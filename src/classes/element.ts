...
export abstract class Element<T extends Json = Json> {
  ...
  protected static create(json: Json): Element {
    ...
    if (ElementPrompts.is(json)) return new ElementPrompts(json);
...
/**
 * It is an element that represents prompts.
 */
export class ElementPrompts extends ElementItem<JsonPrompts> {
  get value(): Value {
    return this.json.prompts;
  }

  static is(json: Json): json is JsonPrompts {
